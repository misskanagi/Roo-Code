...
3. Get access to the API with the following code:
    ...
    ```ts
    const clineExtension = vscode.extensions.getExtension<ClineAPI>("PaperGames.roo-cline-paper")
    ...
    **Note:** To ensure that the `PaperGames.roo-cline-paper` extension is activated before your extension, add it to the `extensionDependencies` in your `package.json`:

    ```json
    "extensionDependencies": [
        "PaperGames.roo-cline-paper"
    ]
    ```
